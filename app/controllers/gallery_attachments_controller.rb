class GalleryAttachmentsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @gallery_attachment.save
        ...
        format.json { render action: 'show', status: :created, location: @gallery_attachment }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to gallery_attachments_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def gallery_attachment_params
      params.require(:gallery_attachment).permit(:gallery_id, :photos)
    end
end
